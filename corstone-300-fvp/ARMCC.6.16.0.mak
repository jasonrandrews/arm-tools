...
COMMA:=,
getvalue=$(word $1,$(subst =, ,$(entry)))
DEFKEYS:=$(foreach entry,$(DEFINES),$(call getvalue,1))
DEFVALUES:=$(foreach entry,$(DEFINES),$(eval value=$(call getvalue, 2))$(if $(value),$(value),1))
AS_AA_DEFINES:=$(call qm2ws,$(join $(addprefix --pd?",$(DEFKEYS)),$(addprefix ?SETA?,$(addsuffix ",$(DEFVALUES)))))
AS_CA_DEFINES:=$(call qm2ws,$(join $(addprefix -Wa$(COMMA)armasm$(COMMA)--pd$(COMMA)",$(DEFKEYS)),$(addprefix ?SETA?,$(addsuffix ",$(DEFVALUES)))))
AS_CG_DEFINES:=$(call qm2ws,$(join $(addprefix -Wa$(COMMA)-defsym$(COMMA)",$(DEFKEYS)),$(addprefix =,$(addsuffix ",$(DEFVALUES)))))
...
ifeq ($(SECURE),Secure)
  LD_SECURE:=--import-cmse-lib-out "$(OUT_DIR)/$(TARGET)_CMSE_Lib.o"
endif
